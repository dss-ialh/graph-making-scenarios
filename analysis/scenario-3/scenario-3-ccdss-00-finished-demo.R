...
# path_input  <- "./data-public/raw/scenario-3/PHAC_Infobase_CCDSS_-8586421808967913043.csv" # anxiety & mood
...
l_support <- ds1 %>%
  prep_data_plot_1(
    # set_area      = c("Canada")
    set_area      = c("Canada", "British Columbia","Manitoba","Quebec")
    # set_area      = names_provinces
    # set_area      = setdiff(names_provinces,"Canada")
    ,set_sex       = c("Females", "Males") # "Females" "Males" "Both sexes"
    # ,set_sex       = c("Both sexes")
    # ,set_age_group = c("20-34","35-49","50-64","65-79", "80+","1+")
    # ,set_age_group = c("1+","20-34","35-49","50-64","65-79", "80+")
    ,set_age_group = c("1+","20-34","35-49","50-64","65-79")
    # ,set_age_group = c("1+")
  ) %>%
  make_plot_1(
    measure = "rate"
...
# l_support$graph <- l_support$graph + scale_color_viridis_d(end=.8, option="magma") 
# l_support$graph <- l_support$graph + facet_grid(age_group ~ area )
l_support$graph <- l_support$graph + theme(strip.text.y = element_text(angle = 0, hjust = 0))
# l_support$graph <- l_support$graph + theme(strip.text.x = element_text(angle = 90, hjust = 0))
l_support$graph <- l_support$graph + theme(legend.position = "bottom")

# watch out: if your filename is very long the jpeg device will refuse to cooperate
l_support %>% 
  print_plot_1(
    path_output_folder = "./analysis/scenario-3/prints/demo-1/"
    # ,prefex            = "attempt1"
    ,graph_name        = "140_216_300" # `auto` by default
    # ,graph_name        = "216_279_300" # `auto` by default
    # ,graph_name        = "279_216_300" # `auto` by default
    # options added through `...` into the jpeg() function
    ,width   = 216
    ,height  = 140
    # ,width   = 216
