...
## disclaimer
 ...
 - This essay is more about demonstrating a technique than about prescribing a framework. ([_"Frameworks are like toothbrushes: everyone's got one and no-one wants to use anyone else's"_](https://twitter.com/trishgreenhalgh/status/891674763285942272))
...
View [`./data-unshared/contents.md`](https://github.com/dss-ialh/graph-making-scenarios/tree/master/data-public/raw/scenario-3) for data dictionary and the retrieval specifications. 
<!-- Load the datasets.   -->
...
## graph-1 basic
```{r phase-1-graph-1, echo=echo_chunks, results='show', message=message_chunks,out.width="960px",warning=warning_chunks, out.width = "700px"}
```
...
```{r phase-2-make_plot-1, echo=echo_chunks, results='show', message=message_chunks,fig.width=11, fig.height=7,out.width="960px",warning=warning_chunks}
```
...
```{r  phase-3-prep_data-1, echo=echo_chunks, results='show', message=message_chunks,fig.width=11, fig.height = 4, out.width="960px",warning=warning_chunks}
```
